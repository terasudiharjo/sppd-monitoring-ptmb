...
perjalanan = sum(1 for s in sppd_list if s["status"] == "dalam_perjalanan")
realisasi = sum(1 for s in sppd_list if s["status"] == "realisasi")
closed = sum(1 for s in sppd_list if s["status"] == "closed")

col1.metric("Total SPPD", total_sppd)
col2.metric("✏️ Draft", draft)
col3.metric("💰 Pencairan", pencairan)
col4.metric("✈️ Dalam Perjalanan", perjalanan)
col5.metric("✅ Selesai", closed)
...
with col_left:
    ...
    status_data = {
        "Status": ["Draft", "Pencairan", "Dalam Perjalanan", "Realisasi", "Closed", "Cancelled"],
        "Jumlah": [
            draft, pencairan, perjalanan, realisasi, closed,
            sum(1 for s in sppd_list if s["status"] == "cancelled")
        ]
    }
...
sppd_aktif = [s for s in sppd_list if s["status"] in ["pencairan", "dalam_perjalanan"]]
