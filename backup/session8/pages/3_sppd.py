...
with tab2:
    ...
    if not spd_data_all:
        ...
        if not sppd_dalam_spd:
            ...
            # ── Tombol Print SPPD (hanya untuk draft) ──
            if s["status"] == "draft":
                st.markdown("#### 🖨️ Print SPPD Pencairan")
                st.info("Klik tombol di bawah untuk mencetak SPPD. Status akan otomatis berubah menjadi **PENCAIRAN** dan RKAP akan dipotong.")
                col_print1, col_print2 = st.columns(2)
                with col_print1:
                    if st.button("🖨️ Print SPPD (Auto → Pencairan)", 
                                 use_container_width=True, type="primary",
                                 key=f"btn_print_{s['id']}"):
                        # Update status ke pencairan
                        db.table("sppd").update({"status": "pencairan"})\
                            .eq("id", s["id"]).execute()
                        # Deduct RKAP
                        ...
                        st.success("✅ Status berubah ke PENCAIRAN. Silakan print SPPD.")
                        # Update label di session_state supaya dropdown tidak reset
                        nama_pegawai = s["pegawai"]["nama"] if s.get("pegawai") else "-"
                        new_label = f"{nama_pegawai} — PENCAIRAN"
                        if new_label in sppd_keys:
                            st.session_state[state_key] = new_label
                        st.rerun()
                with col_print2:
                    st.caption("*(Atau gunakan Update Status manual di bawah)*")
